/**
 * Firestore Database Seeding Script (Using Web SDK)
 *
 * This script populates the Firestore database with initial product data using the Firebase Web SDK.
 * This is simpler than the Admin SDK version as it doesn't require service account credentials.
 *
 * Usage:
 *   1. Install ts-node if not already installed: npm install -g ts-node
 *   2. Run the script: npx ts-node scripts/seed-firestore-web.ts
 *
